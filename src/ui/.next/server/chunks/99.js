...
/* harmony export */   "q2": () => (/* binding */ queryDrops),
/* harmony export */   "qA": () => (/* binding */ queryBalance),
/* harmony export */   "sM": () => (/* binding */ queryDrop)
/* harmony export */ });
/* unused harmony exports queryStats, queryClaimed */
...
const queryStats = async (dropID, host)=>{
    const code = `
  ...
  `.replace(DrizzlePath, publicConfig.drizzleAddress).replace(CloudPath, publicConfig.cloudAddress);
...
const queryClaimed = async (dropID, host)=>{
    const code = `
  import Cloud from 0xCloud
...
const queryClaimStatus = async (dropID, host, claimer)=>{
    const code = `
  import Cloud from 0xCloud
  ...
  `.replace(DrizzlePath, _publicConfig__WEBPACK_IMPORTED_MODULE_0__/* ["default"].drizzleAddress */ .Z.drizzleAddress).replace(CloudPath, _publicConfig__WEBPACK_IMPORTED_MODULE_0__/* ["default"].cloudAddress */ .Z.cloudAddress);
...
const queryDrop = async (dropID, host)=>{
    const code = `
  import Cloud from 0xCloud
  ...
  `.replace(CloudPath, _publicConfig__WEBPACK_IMPORTED_MODULE_0__/* ["default"].cloudAddress */ .Z.cloudAddress);
    const drop = await _onflow_fcl__WEBPACK_IMPORTED_MODULE_1__.query({
        cadence: code,
        args: (arg, t)=>[
                arg(dropID, t.UInt64),
                arg(host, t.Address)
...
const queryBalance = async (token, address)=>{
  ...
  `.replace(FungibleTokenPath, _publicConfig__WEBPACK_IMPORTED_MODULE_0__/* ["default"].fungibleTokenAddress */ .Z.fungibleTokenAddress);
    const balance = await _onflow_fcl__WEBPACK_IMPORTED_MODULE_1__.query({
        cadence: code,
        args: (arg, t)=>[
                arg(address, t.Address)
            ]
    });
    return new (decimal_js__WEBPACK_IMPORTED_MODULE_2___default())(balance ?? 0.0);
};
const queryDrops = async (address)=>{
    const code = `
  import Cloud from 0xCloud
  ...
  `.replace(CloudPath, _publicConfig__WEBPACK_IMPORTED_MODULE_0__/* ["default"].cloudAddress */ .Z.cloudAddress);
    const drops = await _onflow_fcl__WEBPACK_IMPORTED_MODULE_1__.query({
        cadence: code,
        args: (arg, t)=>[
                arg(address, t.Address)
            ]
