...
const NotificationContent = ({ txStatus }) => {
        ...
        <div className="ml-3 w-0 flex-1 pt-0.5">
          <div className="flex gap-x-2">
            ...
            <a
              ...
              target="_blank" className="truncate font-medium font-flow text-sm underline decoration-drizzle-green decoration-2">
              {`${txStatus.txid}`}
            </a >
          </div>
  ...
  if (txStatus.status == "Success") {
    return (
      <>
        <div className="flex-shrink-0">
          <CheckCircleIcon className="h-6 w-6 text-drizzle-green" aria-hidden="true" />
        </div>
        <div className="ml-3 w-0 flex-1 pt-0.5">
          <div className="flex gap-x-2">
